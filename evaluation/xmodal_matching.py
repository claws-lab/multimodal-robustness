...
with open('./dumb_embs/embeddings_test_urls.pickle', 'rb') as handle:
...
for var in range(len(train_ids)):
	...
	# train_y[var, my_labels.index(id2label_dict[tweet_id])] = 1.0 
	train_y[var] = 1


train_x_1 = np.zeros((len(train_ids), train_img_emb.shape[1] + train_text_emb.shape[1]))
train_y_1 = np.zeros((len(train_ids),))
for var in range(len(train_ids)):
	tweet_id = train_ids[var]
	for elt in range(train_img_emb.shape[0]):
		if train_img_id[elt] == tweet_id:
			train_x_1[var, : train_img_emb.shape[1]] = train_img_emb[elt]
	possible_ids = [train_text_id[elt] for elt in range(train_text_emb.shape[0]) if train_text_id[elt] != tweet_id]
	negative_sample_id = random.sample(possible_ids, 1)
	for elt in range(train_text_emb.shape[0]):
		if train_text_id[elt] == negative_sample_id:
			train_x_1[var, train_img_emb.shape[1]:] = train_text_emb[elt]
	train_y_1[var] = 0


train_x_2 = np.zeros((len(train_ids), train_img_emb.shape[1] + train_text_emb.shape[1]))
train_y_2 = np.zeros((len(train_ids),))
for var in range(len(train_ids)):
	tweet_id = train_ids[var]
	for elt in range(train_img_emb.shape[0]):
		if train_img_id[elt] == tweet_id:
			train_x_2[var, : train_img_emb.shape[1]] = train_img_emb[elt]
	possible_ids = [train_text_id[elt] for elt in range(train_text_emb.shape[0]) if train_text_id[elt] != tweet_id]
	negative_sample_id = random.sample(possible_ids, 1)
	for elt in range(train_text_emb.shape[0]):
		if train_text_id[elt] == negative_sample_id:
			train_x_2[var, train_img_emb.shape[1]:] = train_text_emb[elt]
	train_y_2[var] = 0

train_x_3 = np.zeros((len(train_ids), train_img_emb.shape[1] + train_text_emb.shape[1]))
train_y_3 = np.zeros((len(train_ids),))
for var in range(len(train_ids)):
	tweet_id = train_ids[var]
	for elt in range(train_img_emb.shape[0]):
		if train_img_id[elt] == tweet_id:
			train_x_3[var, : train_img_emb.shape[1]] = train_img_emb[elt]
	possible_ids = [train_text_id[elt] for elt in range(train_text_emb.shape[0]) if train_text_id[elt] != tweet_id]
	negative_sample_id = random.sample(possible_ids, 1)
	for elt in range(train_text_emb.shape[0]):
		if train_text_id[elt] == negative_sample_id:
			train_x_3[var, train_img_emb.shape[1]:] = train_text_emb[elt]
	train_y_3[var] = 0

train_x = np.append(train_x, train_x_1, axis = 0)
train_y = np.append(train_y, train_y_1, axis = 0)

train_x = np.append(train_x, train_x_2, axis = 0)
train_y = np.append(train_y, train_y_2, axis = 0)

train_x = np.append(train_x, train_x_3, axis = 0)
train_y = np.append(train_y, train_y_3, axis = 0)
...
if __name__ == "__main__":
	...
	with torch.no_grad():
		...
		for var in range(test_x.shape[0]):
			...
			ground_truth.append(true_label.item())
			# print()
			sim_scores.append(torch.max(torch.nn.functional.softmax(mlp(data))).item())
			curr_pred =  np.argmax(mlp(data)).item()
			predictions.append(curr_pred)
		print(compute_metrics(predictions, ground_truth))


print(sum(sim_scores)/float(len(sim_scores)))
print(np.std(sim_scores))
